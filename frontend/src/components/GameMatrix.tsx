...
const GameMatrix: React.FC = () => {
  ...
  if (gameState.round === 0 || !gameState.gameMatrix.length) {
    return null;
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 w-full max-w-md">
        ...
        <table className="min-w-full border-collapse">
          <thead>
            <tr>
              ...
              {gameState.cellTypes.map((_, colIndex) => (
                <th 
                  key={colIndex} 
          ...
          <tbody>
            {gameState.gameMatrix.map((row, rowIndex) => (
              <tr key={rowIndex}>
      ...
      <div className="mt-6">
        <h3 className="text-lg font-semibold mb-3 text-slate-700">Location Types</h3>
        <div className="grid grid-cols-2 gap-2">
          {gameState.cellTypes.map((type, index) => (
            ...
            >
              <div 
                className={`w-4 h-4 rounded-full mr-2 ${getCellTypeColor(type)}`}
              ></div>
              <span className="text-sm">
                Place {index + 1}: {type.charAt(0).toUpperCase() + type.slice(1)}
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="mt-6">
        <h3 className="text-lg font-semibold mb-3 text-slate-700">Probabilities</h3>
        <div className="space-y-3">
          {gameState.probabilities.map((prob, index) => (
            <div key={index}>
